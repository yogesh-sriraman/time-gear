...
public class CPC_CameraPath : MonoBehaviour
{
    ...
    IEnumerator FollowPath(float time)
    {
        ...
        while (currentWaypointIndex < points.Count)
        {
            ...
            while (Mathf.Abs(currentTimeInWaypoint) < 1)
            {
                if (!paused)
                {
                    pd = 1;
                    if (GameController.instance != null)
                        pd = -GameController.instance.cameraFollow.panDir;

                    currentTimeInWaypoint += (Time.deltaTime * pd)/ timePerSegment;
                    selectedCamera.transform.position = GetBezierPosition(currentWaypointIndex, currentTimeInWaypoint);
                    if (!lookAtTarget)
                        selectedCamera.transform.rotation = GetLerpRotation(currentWaypointIndex, currentTimeInWaypoint);
                    else
    ...
    public int GetNextIndex(int index)
    {
        ...
        return (int)(points.Count + (index + pd)) % points.Count;
    }
    ...
    Vector3 GetBezierPosition(int pointIndex, float time)
    {
        ...
        if (time >= 0)
        {
            t = points[pointIndex].positionCurve.Evaluate(time);
            lerpDir = Vector3.Lerp(
                    Vector3.Lerp(
        ...
        {
            t = points[nextIndex].positionCurve.Evaluate(1 + time);
            lerpDir = Vector3.Lerp(
                Vector3.Lerp(
                    Vector3.Lerp(points[nextIndex].position,
                        points[nextIndex].position + points[nextIndex].handlenext, t),
                    Vector3.Lerp(points[nextIndex].position + points[nextIndex].handlenext,
                        points[pointIndex].position + points[pointIndex].handleprev, t), t),
                Vector3.Lerp(
                    Vector3.Lerp(points[nextIndex].position + points[nextIndex].handlenext,
                        points[pointIndex].position + points[pointIndex].handleprev, t),
                    Vector3.Lerp(points[pointIndex].position + points[pointIndex].handleprev,
                        points[pointIndex].position, t), t), t);
        }
